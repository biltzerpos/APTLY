#!/bin/sh
# 08/11/2018
# This code is used to apply a cut filter to the audio tracks to a given value.
# It needs 3 arguments as input. First argument is input directory which contain wav files.
# Second argument is output directory that you want to create to store processed files.Third argument is 
# the value of the frequency you will use for a highpass filter.See SOX documentation for more details: http://sox.sourceforge.net/sox.html   [See at SINC]
...
if test $# -ne 3
        then
        echo "%cutfilter"
	echo "Usage: cutfilter.sh <indir> <outdir> <frequency>"
...
have wav files"
	...
	echo "Enter the value to which you want to apply the frequency cut filter to the tracks as third argument"
        exit 0
fi            
...
do
	...
	then
       		sox "${dir%/}" "$outdir"/"${subDir%.wav}".wav sinc $rateValue
      	fi
	...
	then
		...
		do
			file=$(basename "$track")
			sox "$track" "$outdir"/"$subDir"/"${file%.wav}".wav sinc $rateValue
		done
	fi
done
