#!/bin/sh
# 07/11/2018
# This code is used to apply a lowpass filter to the audio tracks to a given value.
...
# the value of the frequency you will use for a lowpass filter.
...
if test $# -ne 3
        then
        echo "%lowpassfilter"
	echo "Usage: lowpassfilter.sh <indir> <outdir> <frequency>"
...
have wav files"
	...
	echo "Enter the value to which you want to apply the lowpass filter to the tracks as third argument"
        exit 0
fi            
...
echo Please enter a valid number for lowpass frequency. 
exit 0
fi 
...
do
	...
	then
       		sox "${dir%/}" "$outdir"/"${subDir%.wav}".wav lowpass $rateValue
      	fi
	...
	then
		...
		do
			file=$(basename "$track")
			sox "$track" "$outdir"/"$subDir"/"${file%.wav}".wav lowpass $rateValue
		done
	fi
done
